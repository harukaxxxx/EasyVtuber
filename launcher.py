...
def launch():
    ...
    args = {
        ...
        "auto_mouth": auto_mouth.get(),
        "auto_mouth_method": auto_mouth_method.get(),
    ...
    if p is not None:
        ...
        if args["input"] == 0:
            ...
            run_args.append(
                "0,0," + str(root.winfo_screenwidth()) + "," + str(root.winfo_screenheight())
            )
        elif args["input"] == 4:
            if len(args["osf"]):
                run_args.append("--osf")
        ...
        if args["auto_mouth"] is True:
            run_args.append("--auto_mouth")
            run_args.append("--auto_mouth_method")
            run_args.append(["microphone", "wasapi"][args["auto_mouth_method"]])
...
ttk.Radiobutton(frameL, text="iFacialMocap", value=0, variable=input, command=inputChange).pack(
    fill="x", expand=True
)
ttk.Radiobutton(frameL, text="OpenSeeFace", value=4, variable=input, command=inputChange).pack(
    fill="x", expand=True
)
ttk.Radiobutton(frameL, text="Webcam(opencv)", value=1, variable=input, command=inputChange).pack(
    fill="x", expand=True
)
ttk.Radiobutton(frameL, text="Mouse Input", value=3, variable=input, command=inputChange).pack(
    fill="x", expand=True
)
ttk.Radiobutton(
    frameL, text="Initial Debug Input", value=2, variable=input, command=inputChange
...
ttk.Radiobutton(frameR, text="Highest", value=6, variable=cache_simplify).pack(
    fill="x", expand=True
)
...
ttk.Checkbutton(frameL, text="Extend Movement", variable=is_extend_movement).pack(
    fill="x", expand=True
)
...
ttk.Label(frameL, text="Auto Mouth").pack(fill="x", expand=True)
auto_mouth = tk.BooleanVar(value=args["auto_mouth"])
ttk.Checkbutton(frameL, text="Enable", variable=auto_mouth).pack(fill="x", expand=True)
auto_mouth_method = tk.IntVar(value=args["auto_mouth_method"])
ttk.Radiobutton(frameL, text="microphone", value=0, variable=auto_mouth_method).pack(
    fill="x", expand=True
)
ttk.Radiobutton(frameL, text="wasapi", value=1, variable=auto_mouth_method).pack(
    fill="x", expand=True
)
...
threshold_entry = ttk.Entry(
    frameL,
    textvariable=auto_mouth_threshold,
    validate="key",
...
ttk.Radiobutton(frameL, text="OBS Virtual Camera", value=1, variable=output).pack(
    fill="x", expand=True
)
...
def handle_focus(event):
    ...
    if event.widget == root:
        for item in sorted(
            os.listdir(dirPath), key=lambda x: -os.path.getmtime(os.path.join(dirPath, x))
        ):
            if ".png" == item[-4:]:
                characterList.append(item[:-4])
